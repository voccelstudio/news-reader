...
export default function NewsDashboard() {
  ...
  const loadFeeds = useCallback(async () => {
    ...
    try {
      ...
      setItems(responses.flat().sort((a, b) => new Date(b.pubDate) - new Date(a.pubDate)));
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{ padding: "4rem 2rem", maxWidth: 1200, margin: "0 auto", fontFamily: 'sans-serif' }}>
      <header style={{ marginBottom: "5rem", borderBottom: "2px solid #000", paddingBottom: "1.5rem" }}>
        <h1 style={{ fontSize: 56, fontWeight: 800, margin: 0, letterSpacing: "-3px", lineHeight: 0.8 }}>
          VOC <span style={{ fontWeight: 200 }}>CEL</span>
        </h1>
        <p style={{ margin: "10px 0 0 0", color: "#999", fontSize: 12, textTransform: 'uppercase', letterSpacing: '2px' }}>
          Architecture & Industrial Design Feed
        </p>
      </header>
        ...
        {items.map((it, i) => (
          <article key={i} style={{ display: "flex", flexDirection: "column", gap: 15 }}>
            ...
            <a href={it.link} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', color: '#000' }}>
              <h2 style={{ fontSize: 22, margin: 0, lineHeight: 1.2, fontWeight: 600 }}>{it.title}</h2>
            </a>
            <p style={{ fontSize: 14, color: "#555", lineHeight: 1.5, margin: 0 }}>{it.desc}...</p>
            <div style={{ marginTop: "auto", paddingTop: 15, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 10, color: "#bbb" }}>{new Date(it.pubDate).toLocaleDateString()}</span>
              <a href={it.link} target="_blank" rel="noreferrer" style={{ color: '#000', textDecoration: 'none', fontSize: 11, fontWeight: 800, borderBottom: '2px solid #000' }}>LEER</a>
            </div>
          </article>
        ))}
      </div>
